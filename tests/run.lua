...
local copied_name = vim.fn.tempname():match("[^/]+$")
local copied = "/tmp/" .. copied_name
local previous_cwd = vim.fn.getcwd()
vim.fn.mkdir(copied, "p")
actions.copy_relative({ copied .. "/nested.txt" }, "z", "/tmp")
assert(vim.fn.getreg("z") == copied_name .. "/nested.txt")
actions.copy_relative({ root .. "/tests/run.lua" }, "z", "/tmp")
